...
export function Feed({ events, reports, limit = 12, avatars }: { events: PublicEvent[]; reports: CommunityReport[]; limit?: number; avatars?: Map<string, string | null> }) {
  const now = useNow(15_000);
  const avatarOf = (id: string, name: string) => (avatars ? <Avatar src={avatars.get(id) ?? null} name={name} size={28} className="mt-0.5" /> : null);
  const items: Item[] = [
    ...events.map((event) => ({
      key: `e-${event.id}`,
      at: event.createdAt,
      node: (
        <Link to={`/streamers/${event.streamerLogin}`} className="flex items-start gap-3 rounded-lg border border-border bg-surface px-3 py-2 text-sm hover:border-accent/60">
          {avatarOf(event.streamerId, event.streamerDisplayName)}
          ...
          <span className="min-w-0 flex-1">
            ...
            {event.kind === "goal_added" && <span>a ajouté un goal</span>}
            {event.kind === "goal_updated" && <span>a modifié un goal</span>}
    ...
    ...reports.map((report) => ({
      ...
      node: (
        <Link to={report.streamerLogin ? `/streamers/${report.streamerLogin}` : "/community"} className="flex items-start gap-3 rounded-lg border border-gold/40 bg-surface px-3 py-2 text-sm hover:border-gold">
          {report.streamerDisplayName ? avatarOf(report.streamerId, report.streamerDisplayName) : null}
